...
export const MM2H_FAQS = [
  ...
  {
    q_ko: '에이전시는 꼭 필요한가?',
    a_ko: 'MM2H 는 사실상 에이전시 필수 (정부가 라이선스 에이전시 통한 신청 권장). Thailand Privilege 도 공식 파트너 에이전시를 거치는 경우가 일반적. Singapore EP / PR 은 회사가 처리 (개인 신청은 거의 없음). 에이전시 선택이 비용·통과율에 큰 영향을 주므로 운영자 검증 추천 에이전시를 참고하세요.',
    q_zh: '一定要通过中介申请吗?',
