...
## 🚨 최우선 원칙: 구글 상위노출 — 2026 최신 정책 가이드

**모든 의사결정·기능·콘텐츠는 다음 우선순위에 따라 평가한다.**
**아래 정책과 충돌하는 어떤 결정도 내리지 않는다.**

### 1. E-E-A-T (2022 'Experience' 추가)
- 2022년 말 기존 E-A-T 에 **'경험(Experience)'** 추가됨. 실제 제품 사용·장소 방문 등 직접 경험에서 나온 콘텐츠를 중시.
- 본 사이트는 **YMYL 카테고리** (자녀 교육·이주·재정 결정) → E-E-A-T 신호 비중이 훨씬 더 크게 작용.
- 모든 페이지에 **작성자 바이라인** + **Person schema** (운영자 신원 명시).
- /about/ 페이지에 운영자 이력·자녀 학교 재학 사실·이주 경험 공개.
- 외부 인용·언급 같은 신뢰 신호 누적 (전문가 인터뷰·언론 보도 등).

### 2. 도움되는 콘텐츠 시스템 (2024.3 핵심 알고리즘 통합)
- 2024년 3월부터 별도 업데이트가 아닌 **핵심 랭킹 알고리즘에 통합**됨.
- **사이트 전체 신호** — 저품질 콘텐츠가 일정량 존재하면 양질 페이지까지 순위 하락.
- 저품질 정보 페이지 발행 자체 금지 (1,500자 미만 ❌).
- 빈 카테고리·"준비 중" 페이지 만들지 않음 → 메뉴에서 제외.
- 분기별 트래픽 0 페이지 검토·삭제.

### 3. 독자적 관점 · 1차 경험 (구글 공식 용어)
- 'Information Gain' 은 비공식 표현. 구글 공식 표현은 **"독자적 관점, 1차 경험 리뷰, 비범용 콘텐츠"**.
- 어디서나 볼 수 있는 요약 정보가 아닌 **다른 곳에서 얻을 수 없는 원본 데이터와 직접 경험** 점점 더 중요.
- 모든 페이지에 운영자 1인칭 분석 또는 1차 경험 한 단락 필수.
- 학부모만 아는 비공식 정보 적극 발굴 (한·중 비율, 시설 공유, 노후도, 주변 콘도 시세 등).
- 콘텐츠 발행 전 체크리스트:
...
### 4. 'Who, How, Why' 원칙 (2025.12 갱신)
- 2025년 12월 갱신된 가이드라인은 **'누가, 어떻게, 왜 만들었는가'** 를 기준으로 평가.
- **AI 사용 여부 자체보다는 결과물의 가치가 핵심.**
- AI 를 사용해도 **전문가 검수 + 원본 데이터 + 명시된 책임 저자**가 있다면 문제되지 않음.
- /editorial-policy/ 페이지 필수 (편집 원칙·AI 사용 범위·광고 분리 정책).
- 광고 콘텐츠는 "SPONSORED" 배지로 명확 표시.
- 콘텐츠 출처·검증 방법·업데이트 정책 공개.

### 5. 페이지 경험 — 기술 SEO 기초 (FID → INP 교체)
- Core Web Vitals — **FID 가 INP 로 교체**됨 (Interaction to Next Paint).
- 목표: **LCP < 2.5s, INP < 200ms, CLS < 0.1**.
- 모바일 친화성·로딩 속도·HTTPS 가 기본 요건.
- 모바일 우선 (트래픽 70~80% 모바일).
- schema.org 구조화 데이터 (페이지 유형별).
- hreflang 태그 (ko/zh 상호 매칭).

### 6. 스팸 정책 — 절대 금지 (레드라인)
- **대량 저가치 콘텐츠 양산** (검색 순위 조작 목적) ❌
- **제3자 콘텐츠로 도메인 신뢰도 악용** ❌
- **만료 도메인 인수 후 무관 콘텐츠 발행** ❌
- 키워드 스터핑 ❌
- 클로킹 ❌
- 링크 스킴·대량 백링크 ❌
- 도어웨이 페이지 ❌
...
### 7. 2026.3 코어 + 스팸 업데이트 동시 진행 학습
- 2026.3.24 스팸 업데이트 시작 → 불과 3일 만인 3.27 코어 업데이트 시작 (약 2주간 함께 진행).
- 랭킹 변동이 매우 심했고, 트래픽 감소 원인 분석이 어려운 구간 발생.
- **본 사이트는 처음부터 두 정책 모두 준수**해서 미래의 동시 업데이트에도 영향 최소화.

### 8. 구조화 데이터 확장 (2026.3.24)
- 구글이 **논의 포럼·Q&A 페이지 마크업**에 새로운 지원 속성 추가.
- 댓글 스레드 구조를 더 명확히 이해 → 커뮤니티 사이트 검색 결과 노출 강화.
- 본 사이트에 추후 학부모 Q&A·후기 섹션 추가 시 **`DiscussionForumPosting`** 스키마 적용.

### 9. 선호 썸네일 이미지 지정 (2026.3.2)
- schema.org 마크업 + og:image 메타 태그를 **모두 활용**해 선호 썸네일 지정 가능.
- 구글이 임의로 썸네일 크롤링하지 않도록 명확히 지정.
- **구글 뉴스·Discover 노출에서 특히 중요.**
- BaseLayout 에 schema image + og:image 양쪽 모두 일치하게 자동 생성 (이미 적용).

### 10. JavaScript 정책 완화
- 구글이 **"JavaScript 로 콘텐츠를 로드하는 것이 Google Search 를 더 어렵게 만들지 않는다"** 공식 입장.
- 과거의 SSR 강제론은 해소됨.
- 단, **렌더링 지연·성능 문제는 별도 관리** (Core Web Vitals 영향).
- 본 사이트는 Astro 정적 빌드 → 이 문제 없음.

### 11. 구글 뉴스 상위노출 — 일반 SEO와 다름
- 별도 신청 없이 기준에 맞는 사이트를 자동으로 발견·포함.
- **순수 홍보성 콘텐츠·보도자료·광고성 기사는 절대 노출 안 됨.**
- 독립적이고 사실에 기반한 현재 사건 보도여야 함.
- **저자 바이라인·회사 소개·연락처·편집 정책 명확 공개** 필수.
- 과거 필수였던 AMP 중요도는 크게 낮아짐 → 속도·모바일 최적화가 더 중요.
